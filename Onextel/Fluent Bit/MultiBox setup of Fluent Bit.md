...
2. Create the `fluent-bit.yml` in the folder `~/onextel/fluent-bit_api_hub/config` :
...
```bash
/opt/fluent-bit/bin/fluent-bit -c ~/onextel/fluent-bit_api_hub/config/fluent-bit.yml
```

4. `Ctrl+C` to exit the running service.

5. Create `log-paths.yml` file in the folder `~/onextel/fluent-bit_api_hub/config` :

```yaml
## This file is used to set the log paths for different environments. 
env:
  DLR_LOG_PATH: ~/onextel/fluent-bit_api_hub/logs/DlrSend_*.log
  DLVR_ERROR_CODE_LOG_PATH: ~/onextel/fluent-bit_api_hub/logs/*_dlvr_error_code.log
```

6. Create `parsers.conf` file in the folder `~/onextel/fluent-bit_api_hub/config` :
...
9. Add the `lua` scripts in the folder `~/onextel/fluent-bit_api_hub/lua` :
...
10. Edit the `fluent-bit.yml` that is created already in the folder `~/onextel/fluent-bit_api_hub/config`

```yaml
includes:
  - /home/onexadmin/onextel/fluent-bit_api_hub/config/log-paths.yml
...
service:
  flush: ${FLUSH_INTERVAL}
  log_level: ${LOG_LEVEL}
  parsers_file: /home/onexadmin/onextel/fluent-bit_api_hub/config/parsers.conf
...
pipeline:
  ...
  filters:
    - name: lua
      match: dlr_logs
      script: /home/onexadmin/onextel/fluent-bit_api_hub/lua/process_dlr.lua
      call: process_dlr

    - name: lua
      match: dlvr_error_code_logs
      script: /home/onexadmin/onextel/fluent-bit_api_hub/lua/extract_dlvr_error_code.lua
