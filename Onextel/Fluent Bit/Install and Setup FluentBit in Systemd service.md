...
```bash
sudo systemctl stop fluent-bit.service
```

5. Check for the status of the service, it must be **inactive**:
```bash
sudo systemctl status fluent-bit.service
```

6. Check if the config file is working correctly by executing: 
...
env:
  NGINX_LOG_PATH: /home/onexadmin/onextel/fluent-bit/logs/sbiuatapi.onex-aura.com.log
  DLR_LOG_PATH: /home/onexadmin/onextel/fluent-bit/logs/DlrSend_kS0UZhPy_2025_4_3.log
...
```yaml
includes:
  - /home/onexadmin/onextel/fluent-bit/config/log-paths.yml
...
service:
  ...
  log_level: ${LOG_LEVEL}
  parsers_file: /home/onexadmin/onextel/fluent-bit/config/parsers.conf
...
pipeline:
  ...
  filters:
    - name: lua
      match: nginx_logs
      script: /home/onexadmin/onextel/fluent-bit/lua/filter_invalid_record.lua
      call: filterinvalid_record

    - name: lua
      match: nginx_logs
      script: /home/onexadmin/onextel/fluent-bit/lua/extract_key.lua
      call: extract_key

    - name: lua
      match: dlr_logs
      script: /home/onexadmin/onextel/fluent-bit/lua/process_dlr.lua
      call: process_dlr

    - name: lua
      match: dlvr_error_code_logs
      script: /home/onexadmin/onextel/fluent-bit/lua/extract_dlvr_error_code.lua
...
**TODO**: LASTLY EDIT THE UNIT FILE TO RUN AS A SYSTEM SERVICE 
...
│   └── parsers.conf
└── logs/
    └── DlrSend_kS0UZhPy_2025_4_3.log
```

---

run docker compose: 
1. Compose file : `docker-compose-system.yml`
```bash
...
**IMPORTANT COMMAND**
```bash
/opt/fluent-bit/bin/fluent-bit -c  /home/onexadmin/onextel/fluent-bit_api_hub/config/fluent-bit.yml
```
...
kill the service running on 2020.
...
[Service]
ExecStart=/opt/fluent-bit/bin/fluent-bit -c /home/onexadmin/onextel/fluent-bit_api_hub/config/fluent-bit.yml
Restart=on-failure
...
- Check this to confirm the service is stopped: 
```bash
sudo lsof -i :2020
```
...
Uninstall the fluent-bit
```bash
sudo apt remove --purge fluent-bit -y
...
sudo rm -f /etc/systemd/system/fluent-bit.service
sudo userdel fluent
```

reload to apply service removal: 
```bash
